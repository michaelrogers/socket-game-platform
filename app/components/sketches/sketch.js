...
p.setup = function() {
    ...
    // cord
    p.line(pivot_x, pivot_y, 300, 250);
...
p.draw = function() {
    ...
    p.ellipse(250, 20, 5, 5);
    // reference equilibrium point - not needed
    p.ellipse(250, len + 20, 5, 5);
    ...
    if (hits >= 7) {
        ...
        pinata.velocity.x = 0;
        pinata.velocity.y = 0;
        p.line(pivot_x, pivot_y, pivot_x, pivot_y + len);
...
var updateSpring = function(mag, alpha) {
...
p.pinataSwing = function(displacement) {
    ...
    p.line(pivot_x, pivot_y, pinata.position.x + random(-1, 1), pinata.position.y);
...
p.Candy = function(batForce, img) {
  
  this.pos = p.createVector(pivot_x - 10, pivot_y + len + 40);
  // this.img = sweet;
  
  this.vel = p.createVector(random(-1, 0.2), random(-2,0));
  
  this.vel.mult(random(0, 3))
  this.acc = p.createVector(0, 0);
  var grav = p.createVector(0, 0.1);
  ...
  this.show = function() {
    ...
    p.ellipse(this.pos.x, this.pos.y, random(7, 11), random(5, 9));
